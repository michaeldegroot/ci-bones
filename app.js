  ...
  async.series([
      ...
      function(callback){
        if(fs.existsSync("package.json")){
          ...
          exec('npm init', function(code, output){
            exitIfFailure(code,output)
            setTimeout(callback,100)
          })
        }
      ...
      function(callback){
        if(fs.existsSync("package.json")){
          prompt("Found existing package.json. Do you want to overwrite","confirm",false,function(a){
            if(a){
              exec('npm init', function(code, output){
                exitIfFailure(code,output)
                setTimeout(callback,100)
      ...
      function(callback){
        fs.readFile("package.json", function (err, data) {
          ...
          for(i=0;i<installApps.length;i++){
            if(data.indexOf('"'+installApps[i]+'":') <= 0){
